# -*- coding: utf-8 -*-
"""Copy of han_plain.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1SqzdSq3UFJGh5WYa9-tya6AtF5VgUXCj
"""
...
def read_data(file_name, vocab=None):
    ...
    """
    dir_ = './data'
    df = pd.read_csv(open(dir_+file_name, 'rU'), encoding='utf-8', engine='python', dtype={ 'stars': np.float64, 'text': 'str', 'review_id': 'str' })
    df.dropna(inplace=True)
    df.drop(df.index[300000:], inplace=True)
    ...
    if file_name == 'train_sample.csv':
        fin = io.open(dir_+'crawl-300d-2M.vec', 'r', encoding='utf-8', newline='\n', errors='ignore')
...
def load_data():
    ...
    train_id_list, train_data_label, train_data_matrix, vocab, fastText_embeddings = read_data("train_sample.csv")
...
fin = io.open('./data/crawl-300d-2M.vec', 'r', encoding='utf-8', newline='\n', errors='ignore')
n, d = map(int, fin.readline().split())
fastText_embeddings = np.zeros((len(vocab_dict), embedding_size))
for line in fin:
    tokens = line.rstrip().split(' ')
...
history = model.fit(train_data_matrix, train_data_label, epochs=total_epoch, batch_size=batch_size, \
